...
const issues = computed(() => {
  ...
  const attention = new Set([...semFotos, ...semDesc, ...semPreco, ...semBairro].map((p) => p.id))
  return {
...
const lastUpdated = computed(() => {
  const times = list.value.map((p) => new Date(p.updatedAt).getTime()).filter((t) => !Number.isNaN(t))
  return times.length ? Math.max(...times) : 0
})

const pctPublished = computed(() => (stats.value.total ? Math.round((stats.value.published / stats.value.total) * 100) : 0))
...
<template>
  <div>
    ...
    <template v-else>
      <!-- Métricas -->
      <div class="stat-grid">
        <div class="admin-card stat"><span>{{ stats.total }}</span><small>Cadastrados</small></div>
        <div class="admin-card stat"><span>{{ stats.published }}</span><small>Publicados</small></div>
        ...
        <div class="admin-card stat"><span>{{ stats.drafts }}</span><small>Rascunhos</small></div>
        <div class="admin-card stat" :class="{ warn: stats.semFotos }">
          <span>{{ stats.semFotos }}</span><small>Sem fotos</small>
        </div>
      </div>
      ...
      <div class="admin-card quick">
        ...
        <div class="quick-actions">
          ...
          <NuxtLink class="admin-btn ghost" to="/admin/corretores">Corretores</NuxtLink>
          <a class="admin-btn ghost" href="/" target="_blank" rel="noopener">Ver site ↗</a>
      ...
      <div class="two-col">
        <div class="admin-card">
          ...
          <p v-if="!recent.length" style="color: var(--ink-soft)">Nenhum imóvel cadastrado ainda.</p>
          <table v-else class="admin-table recent-table">
            <thead>
              <tr><th>Imóvel</th><th>Tipo</th><th>Finalidade</th><th>Status</th></tr>
            </thead>
            <tbody>
              <tr v-for="p in recent" :key="p.id">
                <td><NuxtLink :to="`/admin/imoveis/${p.id}`" class="rec-link">{{ p.title }}</NuxtLink></td>
                <td>{{ PROPERTY_TYPE_LABELS[p.type] }}</td>
                <td>{{ p.purpose === 'aluguel' ? 'Aluguel' : 'Venda' }}</td>
                <td><span class="pill" :class="{ muted: p.status !== 'active' }">{{ PROPERTY_STATUS_LABELS[p.status] }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="admin-card health">
          ...
          <template v-else>
            <p class="health-warn">🟡 {{ issues.attention }} {{ issues.attention === 1 ? 'imóvel precisa' : 'imóveis precisam' }} de atenção</p>
            <ul class="health-list">
              <li v-if="issues.semFotos">⚠️ {{ issues.semFotos }} sem fotos</li>
              <li v-if="issues.semDesc">⚠️ {{ issues.semDesc }} sem descrição</li>
              <li v-if="issues.semPreco">⚠️ {{ issues.semPreco }} sem preço</li>
              <li v-if="issues.semBairro">⚠️ {{ issues.semBairro }} sem bairro</li>
            </ul>
          </template>
          ...
          <div class="health-meta">
            ...
            <template v-if="lastUpdated"> · atualizado {{ relTime(lastUpdated) }}</template>
          </div>
          <NuxtLink to="/admin/imoveis" class="see-all">Ver catálogo →</NuxtLink>
        </div>
      </div>
